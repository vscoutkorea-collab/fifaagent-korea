...
import { getPaymentRequests, getPaymentSettings } from '../data'
...
export default function MyPage({ onNavigate, currentUser, onLogout }: MyPageProps) {
  if (!currentUser) {
    return (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          {isApproved ? (
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
              <p className="text-gray-500 text-sm mb-3">아직 모의고사를 구매하지 않았습니다.</p>
                  ...
                  <div key={req.id} className="border border-gray-100 rounded-xl p-4">
                    ...
                    <div className="space-y-1.5 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">{PLAN_LABEL[req.plan]}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">금액</span>
                        <span className="font-semibold text-blue-700">{formatAmount(req.amount)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">입금자명</span>
                        <span className="font-semibold">{req.depositorName}</span>
                      </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="bg-blue-50 rounded-xl p-4 space-y-2.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-gray-900">{settings.bankName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">계좌번호</span>
                <span className="font-bold text-gray-900 tracking-wider">{settings.accountNumber}</span>
              </div>
              <div className="flex justify-between">
