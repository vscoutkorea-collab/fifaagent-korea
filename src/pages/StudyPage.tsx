...
import { Heart, MessageSquare, Plus, X, ChevronLeft, Send, Pin, Trash2 } from 'lucide-react'
...
const CATEGORY_MAP = {
  ...
  notice:     { label: '공지사항',   color: 'bg-red-100 text-red-700' },
}

export default function StudyPage({ currentUser, isAdmin = false }: StudyPageProps) {
  ...
  const save = (updated: StudyPost[]) => { setPosts(updated); saveStudyPosts(updated) }
  ...
  const handleLike = (postId: string) => {
    ...
    save(updated)
    if (selected?.id === postId) setSelected({ ...selected, likes: selected.likes + 1 })
  }
  ...
  const handleDeleteComment = (postId: string, commentId: string) => {
    ...
    save(updated)
    if (selected?.id === postId) setSelected({ ...selected, comments: selected.comments.filter((c) => c.id !== commentId) })
  }
  ...
  const handleAddComment = (postId: string) => {
    ...
    save(updated)
    if (selected?.id === postId) setSelected({ ...selected, comments: [...selected.comments, comment] })
    setCommentText('')
  }

  if (selected) {
    const post = posts.find((p) => p.id === selected.id) ?? selected
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <button onClick={() => setSelected(null)} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 mb-6">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm mb-4">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${CATEGORY_MAP[post.category]?.color ?? 'bg-gray-100 text-gray-600'}`}>
              ...
              {isAdmin && (
                <button onClick={() => handleDeletePost(post.id)}
                  className="flex items-center gap-1 text-xs px-3 py-1.5 border border-red-200 text-red-500 rounded-lg hover:bg-red-50 flex-shrink-0">
                  <Trash2 size={12} /> 삭제
                </button>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 text-sm text-gray-500 mb-6 pb-6 border-b border-gray-100">
              <span>{post.author === '관리자' ? <strong className="text-blue-600">관리자</strong> : post.author}</span>
              <span>·</span>
            ...
            <div className="flex items-center gap-4 mt-6 pt-6 border-t border-gray-100">
              ...
              <span className="flex items-center gap-2 text-gray-400 text-sm">
                <MessageSquare size={16} /> {post.comments.length}개 댓글
              </span>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <h2 className="font-bold text-gray-900 mb-4">댓글 {post.comments.length}개</h2>
            <div className="space-y-4 mb-6">
              {post.comments.length === 0 && <p className="text-gray-400 text-sm text-center py-4">첫 댓글을 남겨보세요!</p>}
              {post.comments.map((comment) => (
                <div key={comment.id} className="bg-gray-50 rounded-xl p-4 flex items-start gap-3">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {[{ id: 'all', label: '전체' }, ...Object.entries(CATEGORY_MAP).map(([k, v]) => ({ id: k, label: v.label }))].map((tab) => (
            <button key={tab.id} onClick={() => setFilter(tab.id as StudyPost['category'] | 'all')}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${filter === tab.id ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {sorted.length === 0 && <div className="text-center py-16 text-gray-400">게시물이 없습니다.</div>}
          {sorted.map((post) => (
            <div key={post.id} className={`bg-white rounded-2xl p-6 shadow-sm border hover:shadow-md transition-shadow ${post.pinned ? 'border-red-200' : 'border-gray-100'}`}>
              <div className="flex items-start justify-between gap-3">
                <button onClick={() => setSelected(post)} className="flex-1 text-left min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    {post.pinned && <Pin size={13} className="text-red-500 flex-shrink-0" />}
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${CATEGORY_MAP[post.category]?.color ?? 'bg-gray-100 text-gray-600'}`}>
                      {CATEGORY_MAP[post.category]?.label ?? post.category}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                    <span className={post.author === '관리자' ? 'text-blue-600 font-semibold' : ''}>{post.author}</span>
                    <span>{post.createdAt}</span>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={newPost.category}
                  ...
                  {Object.entries(CATEGORY_MAP).filter(([k]) => k !== 'notice').map(([k, v]) => (
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => setShowWriteForm(false)}
                  className="px-6 py-3 border border-gray-200 rounded-xl text-gray-700 font-medium hover:bg-gray-50">취소</button>
                <button onClick={handleSubmitPost}
                  className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-colors">게시하기</button>
              </div>
            </div>
          </div>
        </div>
